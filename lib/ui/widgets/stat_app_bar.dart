...
//     selectedMonth =  now.year.toString();
//     selectedYear =  '${_monthNames[now.month - 1]}/${now.year}'; // Default to current year
...
//         final month = DateTime(currentYear, index + 1, 1);
...
class _StatAppBarState extends State<StatAppBar> {
  ...
  void initState() {
    super.initState();
    final now = DateTime.now();
    // selectedMonth = '${_monthNames[now.month - 1]}/${now.year}'; // Default to current month
    selectedMonth= now.year.toString();
    selectedYear = now.year.toString();
        ...
        child: SafeArea(
          child: Column(
            ...
            children: [
              _PeriodSelector(),
              const SizedBox(height: 10,),
              // Removed the horizontal list of chips
            ],
          ),
        ),
      ),
